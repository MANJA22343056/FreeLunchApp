...
class _ChatBotScreenState extends State<ChatBotScreen> {
  ...
  void initState() {
    ...
    _model = GenerativeModel(
      ...
      apiKey:
          'AIzaSyCczypulZKnwl6MAI3x87nIb18BRuyBG0U', // Replace with your API key
    );
  ...
  Future<void> _fetchUserRoleAndName() async {
    ...
    if (currentUser != null) {
      try {
        ...
        if (userDoc.exists && userDoc.data() != null) {
          fetchedUserName = (userDoc.data() as Map<String, dynamic>)['name'] ??
              currentUser.email ??
              'Pengguna';
          fetchedUserRole = (userDoc.data() as Map<String, dynamic>)['role'] ??
              'Tidak Diketahui';
        } else {
    ...
    setState(() {
      ...
      _messages.add({
        'role': 'bot',
        'text': 'Halo, $_userName! Apakah ada yang bisa saya bantu?'
      });
    });
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        iconTheme: const IconThemeData(
            color: Colors.white), // Icon color (e.g., back button)
      ),
      body: Column(
        children: [
          Expanded(
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    onSubmitted: (value) =>
                        _sendMessage(), // Panggil _sendMessage saat Enter ditekan
